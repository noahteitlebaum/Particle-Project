...
//File Name: Circular.cs
//Project Name: PASS2
//Creation Date: Oct. 16, 2023
//Modified Date: Oct. 30, 2023
//Description: Building an emitter class that launches its particles inside a given circle area
...
{
    class Circular : Emitter
    {
        //Store the radius max value
        private static int DIMESNSIONS_MAX = 200;

        //Store the data that makes up a circular emitter
        private int radius;
        private Color circleColour;

        //Store the data to draw the circle
        private GraphicsDevice gd;
        private GameCircle gameCirc;

        /// <summary>
        /// Circular acts like a normal Emitter, but it launches from a random position within a given circle
        /// </summary>
        ...
        /// <param name="radius">Radius of the circular emitter</param>
        /// <param name="circleColour">Colour of the circular emitter</param>
        /// <param name="gd">Creates the area of the circular emitter to draw</param>
        public Circular(Texture2D img, float scale, Vector2 pos, int numParticles, int launchTimeMin, int launchTimeMax,
                ...
                int radius, Color circleColour, GraphicsDevice gd) : base(img, scale, pos, numParticles, launchTimeMin, launchTimeMax,
        ...
        {
            //Load the data that makes up and draws a circular emitter
        ...
        {
            //Set the emitter's bounding box based off the position
            rec.X = (int)x;
            rec.Y = (int)y;

            //Create the circular emitter's launch area to be drawn
            gameCirc = new GameCircle(gd, GetPosition(), radius, 1);
        }

        //Pre: None
        //Post: Return a resulting position inside the circles launch area
        //Description: Calculate a random position inside the circles launch area
        private Vector2 GetLaunchPos()
        {
            //Randomize a new radius and angle inside the circles area
            double newRadius = GetFloatInRange(0, radius);
            double theta = Game1.rng.NextDouble() * 2 * Math.PI;
        ...
        public override void Draw(SpriteBatch spriteBatch)
        {
            ...
            if (showLauncher)
            {
                //Draw the circular emitter's launch area
                gameCirc.Draw(spriteBatch, circleColour * 0.6f, Color.Black);
            }
